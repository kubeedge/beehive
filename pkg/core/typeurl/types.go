...
// Package typeurl is Marshal For any type
package typeurl
...
// registry store the reflectType and typeurl string map
var registry map[reflect.Type]string

// revRegistry store the typeurl string and reflectType map
var revRegistry map[string]reflect.Type
var lock sync.Mutex

const (
	baseTypeURL = "kubeedge.io"
)
...
// getRealType get the reflectType of v, if v is Ptr, get it`s real type
func getRealType(v interface{}) reflect.Type {
	t := reflect.TypeOf(v)
	switch t.Kind() {
	case reflect.Ptr:
		// if kind is Ptr, get it`s real type
...
func TypeURL(v interface{}) (string, error) {
	lock.Lock()
	url, ok := registry[getRealType(v)]
	lock.Unlock()
	if ok {
		return url, nil
	}
	panic(errors.Errorf("Can not get typeurl, Type %v not register", getRealType(v)))
}
